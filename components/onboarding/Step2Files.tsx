    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="text-right">
                <h2 className="text-2xl font-bold font-arabic text-white mb-2">
                    {t("onboarding.step2.title")} <span className="text-red-500">*</span>
                </h2>
                <p className="text-slate-400 text-sm font-arabic">{t("onboarding.step2.subtitle")}</p>
            </div>
            ...
            >
                ...
                <h3 className="text-lg font-bold text-white mb-2 font-arabic">{t("onboarding.step2.uploadTitle")}</h3>
                <p className="text-sm text-slate-400 font-arabic">{t("onboarding.step2.uploadDesc")}</p>
            </div>
            ...
            {fileErrors && (
                <div className="p-4 bg-red-500/10 border border-red-500/30 rounded-xl flex items-start gap-3">
                    ...
                    <p className="text-red-400 text-sm font-arabic">{fileErrors}</p>
                </div>
            )}
            ...
            {files.length > 0 && (
                <div className="space-y-3 mt-6">
                    <div className="flex items-center justify-between">
                        <h4 className="text-right text-sm font-bold text-slate-300 font-arabic">
            ...
            {files.length === 0 && (
                <div className="text-center p-4 bg-red-500/10 border border-red-500/30 rounded-xl">
                    <p className="text-red-400 text-sm font-arabic">{t("onboarding.step2.filesRequired")}</p>
                </div>
            )}

            {/* Note about large datasets */}
            <div className="p-4 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                <p className="text-blue-400 text-sm font-arabic">{t("onboarding.step2.note")}</p>
            </div>

            {/* Google Drive Link Section */}
            <div className="space-y-3 pt-4 border-t border-white/10">
                <label className="text-right block">
                    <h3 className="text-lg font-bold text-white mb-1 font-arabic">{t("onboarding.step2.googleDriveSection")}</h3>
                    <p className="text-xs text-slate-400 font-arabic">{t("onboarding.step2.googleDriveLabel")}</p>
                </label>
                <input
                    type="text"
                    dir={lang === "ar" ? "rtl" : "ltr"}
                    ...
                    className={`w-full px-4 py-3 rounded-xl bg-[#111821] border text-white placeholder-slate-500 font-arabic transition-colors outline-none ${
                        googleDriveError
                ...
                {googleDriveError && (
                    <div className="flex items-center gap-2 p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                        ...
                        <p className="text-red-400 text-xs font-arabic">{googleDriveError}</p>
                    </div>
                )}
                {googleDriveLink && !googleDriveError && (
                    <div className="flex items-center gap-2 p-3 bg-green-500/10 border border-green-500/30 rounded-lg">
                        <CheckCircle className="w-4 h-4 text-green-400 flex-shrink-0" />
                        <p className="text-green-400 text-xs font-arabic">{lang === "ar" ? "رابط صحيح ✓" : "Valid link ✓"}</p>
                    </div>
                )}
            </div>
