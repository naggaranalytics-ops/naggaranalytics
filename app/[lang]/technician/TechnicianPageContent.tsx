...
import TechnicianProjectActions from './TechnicianProjectActions';

export default function TechnicianPageContent({ projects, fetchError }: { projects: any[]; fetchError: string | null }) {
            ...
            <div className="space-y-4">
                {projects.length === 0 ? (
                    <div className="text-center py-20 bg-[#111821] border-dashed border border-white/10 rounded-2xl">
                        <p className={`text-slate-400 font-${dir === 'rtl' ? 'arabic' : 'sans'}`}>{t('tech.page.empty')}</p>
                    </div>
                ) : (
                    projects.map((project: any) => (
                        <div key={project.$id} className="bg-[#111821] border border-white/5 rounded-2xl p-6 transition-all hover:border-[#16a085]/30">
                            <div className="flex flex-col xl:flex-row justify-between gap-6" dir={dir}>
                                ...
                                {/* Info — NO customer_id, NO pricing */}
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 mb-2 flex-wrap">
                                                ...
                                                <span className={`px-3 py-0.5 text-xs rounded-full font-medium border font-${dir === 'rtl' ? 'arabic' : 'sans'} ${
                                                    ...
                                                    project.status === 'in_progress'  ? 'bg-[#16a085]/10 text-[#16a085] border-[#16a085]/20' :
                                                                                        'bg-yellow-500/10 text-yellow-400 border-yellow-500/20'
                                                }`}>
                                                    {t(`tech.action.status.${project.status}`)}
                                                </span>
                                            </div>
                                            <p className={`text-slate-400 text-sm font-${dir === 'rtl' ? 'arabic' : 'sans'} mt-1`}>
                                                {t('tech.project.degree')} {project.degree_type === 'Masters' ? t('tech.project.masters') : project.degree_type === 'PhD' ? t('tech.project.phd') : project.degree_type || t('tech.project.unspecified')}
                                            </p>
                                        </div>
                                    </div>

                                    <div className={`grid grid-cols-2 gap-4 ${dir === 'rtl' ? 'text-right' : 'text-left'} pt-4 border-t border-white/5`}>
                                        <div>
                                            <p className={`text-slate-500 text-xs font-${dir === 'rtl' ? 'arabic' : 'sans'} mb-1`}>{t('tech.project.projectId')}</p>
                                            <p className="text-white text-sm font-mono truncate">{project.$id}</p>
                                        </div>
                                        <div>
                                            <p className={`text-slate-500 text-xs font-${dir === 'rtl' ? 'arabic' : 'sans'} mb-1`}>{t('tech.project.submitDate')}</p>
                                ...
                                <div className="xl:w-80 bg-black/20 rounded-xl p-4 border border-white/5 self-stretch flex flex-col justify-between gap-4">
                                    <div className={`flex items-center gap-2 py-2 bg-slate-800/50 text-slate-500 rounded-lg px-3 font-${dir === 'rtl' ? 'arabic' : 'sans'} text-sm`}>
                                        {t('tech.project.filesMsg')}
                                    </div>
                                    <TechnicianProjectActions projectId={project.$id} currentStatus={project.status} />
                                </div>
                            </div>
                        </div>
                    ))
