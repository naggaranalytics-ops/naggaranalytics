...
export default function LoginPage() {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            // Redirect to Kinde's login endpoint with custom parameters
            const loginUrl = new URL(`${process.env.NEXT_PUBLIC_KINDE_ISSUER_URL}/oauth2/auth`);
            loginUrl.searchParams.append("client_id", process.env.NEXT_PUBLIC_KINDE_CLIENT_ID || "");
            loginUrl.searchParams.append("redirect_uri", `${process.env.NEXT_PUBLIC_KINDE_REDIRECT_URL}`);
            loginUrl.searchParams.append("response_type", "code");
            loginUrl.searchParams.append("scope", "openid profile email offline");
            loginUrl.searchParams.append("login_hint", email || "");
            ...
            <main className="flex-1 flex items-center justify-center py-20 px-4 sm:px-6 lg:px-8 mt-20">
                <div className="w-full max-w-md">
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        >
                            {isLoading ? "Loading..." : t("nav.signIn")}
                    ...
                    <div className="mt-8 p-4 rounded-lg border" style={{ backgroundColor: 'var(--bg-secondary)', borderColor: 'var(--border-color)' }}>
                        <p style={{ color: 'var(--text-secondary)' }} className="text-sm">
                            {t("nav.register")}{" "}
                            <LocaleLink href="/auth/signup" className="font-bold text-[#16a085] hover:underline">
                                {t("nav.getStarted")}
                            </LocaleLink>
                        </p>
                    </div>
